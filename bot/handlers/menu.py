...
async def main_menu(event, user: User, **kwargs):
    ...
    if isinstance(event, CallbackQuery):
        ...
        if is_admin_telegram_id(user.telegram_id):
            await event.message.answer(admin_entry_text(lang), reply_markup=admin_entry_keyboard(lang), parse_mode="HTML")
        await event.answer()
    else:
        await event.answer(text, reply_markup=main_menu_keyboard(lang=lang, telegram_id=user.telegram_id), parse_mode="HTML")
        if is_admin_telegram_id(user.telegram_id):
            await event.answer(admin_entry_text(lang), reply_markup=admin_entry_keyboard(lang), parse_mode="HTML")
